...
impl RegisterDeviceParser {
    pub fn parse(args: &Vec<String>) -> Result<Command, Error> {
        let count = args.len();
        if count != 4 {
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Register Device is expecting parameters: username, devicename"));
        }

        let arguments = Vec::from([
            CommandArgument::new(get_arg(args, 2).to_string()), // username
            CommandArgument::new(get_arg(args, 3).to_string()), // devicename
        ]);
