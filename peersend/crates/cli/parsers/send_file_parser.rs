...
impl SendFileParser {
    pub fn parse(args: &Vec<String>) -> Result<Command, Error> {
        let count = args.len();
        if count != 4 {
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Send File is expecting parameters: filepath, target device name"));
        }

        let arguments = Vec::from([
            CommandArgument::new(get_arg(args, 2).to_string()), // filepath
            CommandArgument::new(get_arg(args, 3).to_string()), // target device name
        ]);
